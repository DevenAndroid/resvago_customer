  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: StreamBuilder<List<NotificationData>>(
          ...
          builder: (BuildContext context, AsyncSnapshot<List<NotificationData>> snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return const Center(child: CircularProgressIndicator());
            } else if (snapshot.hasError) {
              return Center(child: Text("Error: ${snapshot.error}"));
            } else if (!snapshot.hasData || snapshot.data!.isEmpty) {
              return const Center(child: Text("Notification not available"));
            } else {
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      item.title.toString(),
                                      style:
                                          const TextStyle(color: Color(0xff384953), fontSize: 12, fontWeight: FontWeight.normal),
                                    ),
                                  ],
                                ),
                              ),
